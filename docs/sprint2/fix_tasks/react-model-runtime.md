...
- [ ] 在 `test_react_model.py` 编写失败测试：OpenAI、DeepSeek、OpenAI-compatible 配置分别生成正确 model、API key、base URL 和 temperature。
- [ ] 编写失败测试：缺少 API key 或不支持 tool calling 时抛出 `ReActModelUnavailableError`，不得回退固定模板。
- [ ] 编写失败测试：测试模式可注入 `FakeMessagesListChatModel`，且能够返回预设 tool call。
- [ ] 运行 `conda run -n carrer_agent pytest -q backend/tests/test_react_model.py`，确认因模块不存在而失败。
- [ ] 添加 `langchain-core`、`langchain-openai` 生产依赖并同步 `requirements-dev.txt`。
- [ ] 实现 `ReActModelUnavailableError` 和 `ReActModelFactory.create(run_config)`；返回支持 `bind_tools` 的 ChatModel。
- [ ] 为 DeepSeek/OpenAI-compatible 设置用户提供的 `base_url`；不得记录 API key。
- [ ] 增加启动前 capability check，无法绑定工具时返回 `REACT_MODEL_UNAVAILABLE`。
- [ ] 再次运行 `conda run -n carrer_agent pytest -q backend/tests/test_react_model.py`，确认全部通过。
- [ ] 运行 `conda run -n carrer_agent pytest -q backend/tests/test_llm_service.py`，确认原有一次性 LLM 调用未回归。
- [ ] 提交：`git commit -m "feat: add tool-calling ReAct model runtime"`。
