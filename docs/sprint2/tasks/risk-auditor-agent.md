...
- [x] 创建 Risk Auditor Agent 模块。
- [x] 使用 LangGraph `create_react_agent` 组装 agent。
- [x] 只暴露 `check_requirement_coverage`、`find_resume_vague_claims`、`check_generated_claim_grounding`、`rank_top_risks` 四个工具。
- [x] 设置最大迭代轮数为 3。
- [x] prompt 要求风险必须具体到 JD 要求和简历现状。
- [x] prompt 要求优先分析项目和实习，不以技能列表为主要依据。
- [x] 输出 schema 限制最多 3 条。
- [x] 编写测试：不展示内部 requirement ID。
- [x] 编写测试：重复泛化风险会被合并或丢弃。
- [x] 编写测试：风险按严重程度和求职影响排序。
